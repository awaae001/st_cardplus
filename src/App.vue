<template>
  <div class="flex h-screen overflow-hidden bg-gray-100 dark:bg-neutral-950">
    <header
      class="lg:hidden p-2 fixed top-0 left-0 z-[1002] bg-white/80 dark:bg-neutral-800/80 backdrop-blur-sm shadow-sm"
    >
      <button
        @click="toggleSidebar"
    ...
    <div
      class="menu-bar-container flex-shrink-0 fixed lg:static inset-y-0 left-0 z-[1001] transform transition-transform lg:transition-[width] duration-300 ease-in-out"
      :class="{
    ...
    >
      <ElMenu
        ...
        class="menu-bar h-full flex flex-col bg-white dark:bg-neutral-800 border-r border-gray-200 dark:border-neutral-700 overflow-x-hidden transition-[width] duration-300 ease-in-out"
        :class="{
      ...
      >
        <div
          class="app-title-container p-4 text-center text-lg font-semibold text-gray-700 dark:text-gray-200 flex-shrink-0 h-[56px] flex items-center justify-center overflow-hidden whitespace-nowrap relative"
        >
          <img
            src="@/image/logo.png"
            alt="Logo"
          ...
          <span
            ...
            :class="{
              'text-collapsed-style': isEffectivelyCollapsedForTextAnimation,
            }"
            >ST CardPlus</span
          >
        </div>
        ...
        <div class="flex-grow overflow-y-auto menu-items-wrapper">
          <ElMenuItem
            index="/"
            :title="isEffectivelyCollapsedForTitle ? '首页' : ''"
          >
            ...
            <template #title>
              <span
                ...
                :class="{
                  'text-collapsed-style':
                    isEffectivelyCollapsedForTextAnimation,
                }"
                >首页</span
              >
            </template>
          ...
          <ElMenuItem
            index="/card"
            :title="isEffectivelyCollapsedForTitle ? '角色卡编辑器' : ''"
          >
            <ElIcon :size="22"
              ><Icon icon="ph:user-circle-gear-light"
            /></ElIcon>
            <template #title>
              <span
                class="menu-item-text"
          ...
          <ElMenuItem
            index="/world"
            :title="isEffectivelyCollapsedForTitle ? '地标编辑器' : ''"
          >
          ...
          <ElMenuItem
            index="/cardoutput"
            :title="isEffectivelyCollapsedForTitle ? '角色卡快搭' : ''"
          >
            <ElIcon :size="22"
              ><Icon icon="ph:identification-card-light"
            /></ElIcon>
            <template #title>
              <span
                class="menu-item-text"
          ...
          <ElMenuItem
            index="/worldbook"
            :title="isEffectivelyCollapsedForTitle ? '世界书编辑' : ''"
          >
          ...
          <ElMenuItem
            index="/toolbox"
            :title="isEffectivelyCollapsedForTitle ? '工具箱' : ''"
          >
        ...
        <div
          class="mt-auto flex-shrink-0 border-t border-gray-200 dark:border-neutral-700"
        >
          <ElMenuItem
            index="action:toggle-autosave"
            @click="appSettings.toggleAutoSave"
            :title="
              isEffectivelyCollapsedForTitle
                ? '自动保存: ' +
                  (appSettings.isAutoSaveEnabled ? '开启' : '关闭')
                : ''
            "
          >
            <ElIcon :size="22" class="text-blue-500">
              <Icon
                v-if="appSettings.isAutoSaveEnabled"
                icon="ph:floppy-disk-back-light"
              />
          ...
          <ElMenuItem
            index="action:cycle-safemode"
            @click="appSettings.cycleSafeMode"
            :title="
              isEffectivelyCollapsedForTitle
                ? '安全模式: ' + safeModeTitle(appSettings.safeModeLevel)
                : ''
            "
          >
            <ElIcon :size="22" class="text-orange-500">
              <Icon
                v-if="appSettings.safeModeLevel === 'forbidden'"
                icon="ph:shield-checkered-light"
              />
              <Icon
                v-else-if="appSettings.safeModeLevel === 'double'"
          ...
          <ElMenuItem
            index="/about"
            :title="isEffectivelyCollapsedForTitle ? '关于' : ''"
          >
    ...
    <main
      class="flex-1 flex flex-col overflow-hidden dark:bg-gradient-to-br dark:from-neutral-950 dark:to-neutral-900 text-neutral-800 dark:text-neutral-300"
      :class="{
        'pt-14 lg:pt-0': true,
      }"
    >
    ...
    <div
      ...
      class="fixed inset-0 z-[1000] bg-black/30 dark:bg-black/40 backdrop-blur-sm lg:hidden transition-opacity duration-300 ease-in-out"
      :class="{
...
const handleMenuNavigation = (index: string) => {
...
watch(
  ...
  (newIsMobile, oldIsMobile) => {
    if (
      newIsMobile === false &&
      (oldIsMobile === true || oldIsMobile === undefined)
    ) {
      isSidebarOpen.value = true;
    } else if (
      newIsMobile &&
      (oldIsMobile === false || oldIsMobile === undefined)
    ) {
      isSidebarOpen.value = false;
    }
  },
...
.menu-item-text {
  ...
  transition: opacity 0.2s ease-in-out 0.1s, transform 0.2s ease-in-out 0.1s,
    max-width 0.2s ease-in-out 0.1s, visibility 0s linear 0.2s;
  white-space: nowrap;
  overflow: hidden;
  max-width: 150px;
  vertical-align: middle;
...
.app-title-text {
  ...
  width: auto;
  transition: opacity 0.2s ease-in-out 0.1s, transform 0.2s ease-in-out 0.1s,
    width 0.2s ease-in-out 0.1s, visibility 0s linear 0.2s;
  white-space: nowrap;
  overflow: hidden;
  vertical-align: middle;
}
