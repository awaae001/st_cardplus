  ...
  <div class="content-panel-body space-y-4 md:space-y-5 h-full flex flex-col">
    <div class="content-panel-header -mx-4 md:-mx-5 -mt-4 md:-mt-5 mb-5">
      ...
      <div class="flex gap-x-3 ml-auto">
        <button @click="handleAddGreeting" class="btn-primary-adv text-sm !py-1.5 !px-3 whitespace-nowrap" title="添加新的备用问候语">
          <Icon icon="material-symbols:add-comment-outline-rounded" width="18" height="18" class="mr-1.5 -ml-0.5" />
          添加问候语
        </button>
      </div>
    </div>
      ...
      >
        <template #item="{ element: greeting }">
          <div :key="greeting.id" class="greeting-item-card-outer-wrapper">
            <div class="greeting-item-card bg-white dark:bg-neutral-800 rounded-md shadow dark:shadow-black/10 border border-neutral-200 dark:border-neutral-700 flex items-start p-2.5 gap-x-2.5 transition-all duration-150 ease-in-out hover:shadow-md dark:hover:border-neutral-600">
              ...
              <div class="flex-grow min-w-0">
                <el-input type="textarea" v-model="greeting.text" :autosize="{ minRows: 2, maxRows: 6 }" placeholder="请输入备用问候语（支持换行）" class="w-full"/>
              </div>
            </div>
          </div>
        </template>
      </draggable>
      <div v-else class="text-center py-10 border-2 border-dashed border-neutral-300/70 dark:border-neutral-700/70 rounded-lg bg-neutral-50 dark:bg-neutral-800/30">
        <Icon icon="ph:chat-circle-dots-duotone" class="text-5xl text-neutral-400 dark:text-neutral-600 mx-auto mb-3" />
        <p class="text-neutral-600 dark:text-neutral-400 text-sm font-medium">暂无备用问候语</p>
        <p class="text-xs text-neutral-500 dark:text-neutral-500 mt-1">点击“添加问候语”开始吧！</p>
      </div>
    </div>
  </div>
</template>
...
watch(() => props.form.data.alternate_greetings, (newValArray) => {
...
<style scoped>
.greeting-ghost { opacity: 0.6; border-radius: var(--radius-md); background-color: var(--color-sky-100); outline: 2px dashed var(--color-sky-400); box-shadow: none; }
:where(.dark, .dark *) .greeting-ghost { background-color: oklch(from var(--color-sky-800) l c h / 0.4); }
.greeting-chosen .greeting-item-card { --chosen-ring-width: 2px; --chosen-ring-offset-width: 1px; --chosen-ring-color-light: var(--color-accent-500); --chosen-ring-offset-color-light: var(--color-white); --chosen-ring-color-dark: var(--color-accent-400); --chosen-ring-offset-color-dark: var(--color-neutral-850); box-shadow: 0 0 0 var(--chosen-ring-offset-width) var(--chosen-ring-offset-color-light), 0 0 0 calc(var(--chosen-ring-offset-width) + var(--chosen-ring-width)) var(--chosen-ring-color-light), var(--shadow-lg); transform: scale(1.01); z-index: 10; }
:where(.dark, .dark *) .greeting-chosen .greeting-item-card { box-shadow: 0 0 0 var(--chosen-ring-offset-width) var(--chosen-ring-offset-color-dark), 0 0 0 calc(var(--chosen-ring-offset-width) + var(--chosen-ring-width)) var(--chosen-ring-color-dark), var(--shadow-lg); }
.greeting-item-card :deep(.el-textarea__inner) { font-size: 0.875rem; line-height: 1.6; }
</style>