  ...
  <div class="content-panel-body space-y-4 md:space-y-5 h-full flex flex-col">
    <div class="content-panel-header -mx-4 md:-mx-5 -mt-4 md:-mt-5 mb-5">
      <h3 class="content-panel-title flex items-center gap-2">
        <Icon icon="ph:chats-teardrop-duotone" class="text-xl text-accent-500 dark:text-accent-400"/>
        对话示例
      </h3>
    </div>
    <div class="flex-grow px-1 flex">
      <el-input type="textarea" v-model="localMesExample" :autosize="{ minRows: 5, maxRows: 12 }" placeholder="请输入对话示例 (例如角色可能会说的话，或与角色的对话片段)" class="flex-grow"/>
    </div>
  </div>
</template>
...
interface DialogueExampleFormData { mes_example: string; data?: { mes_example?: string; [key: string]: any; }; }
interface Props { form: DialogueExampleFormData; }
...
watch(() => props.form.mes_example, (newVal) => {
  ...
  if (currentPropVal !== localMesExample.value) {
    ...
    nextTick(() => { internalUpdateFlag_Dial = false; });
  }
}, { immediate: true });
