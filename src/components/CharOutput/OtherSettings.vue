<template>
  <div class="content-panel-body space-y-4 md:space-y-5 h-full flex flex-col">
    ...
    <div class="flex-grow space-y-5 px-1">
      <el-form-item class="other-settings-form-item">
        <template #label
          ><span class="form-label-adv"
            >发言频率 (Talkativeness)</span
          ></template
        >
        <div class="slider-control-wrapper">
          <el-slider
            v-model="localOtherSettings.talkativeness"
            :min="0"
            :max="1"
            :step="0.01"
            show-input
            class="other-settings-slider"
            :show-tooltip="false"
            :debounce="300"
          />
        </div>
      ...
      <el-form-item class="other-settings-form-item">
        <template #label
          ><span class="form-label-adv">是否收藏 (Favorite)</span></template
        >
        <el-switch
          v-model="localOtherSettings.fav"
          inline-prompt
          :active-icon="CheckIcon"
          :inactive-icon="CloseIcon"
          active-text="是"
          inactive-text="否"
          class="form-item-switch-control"
        />
      </el-form-item>
    </div>
  </div>
...
interface OtherSettingsData {
  ...
  fav: boolean;
  data?: {
    extensions?: { talkativeness?: string; fav?: boolean; [key: string]: any };
    [key: string]: any;
  };
}
interface Props {
  form: OtherSettingsData;
}
...
watch(
  () => [props.form.talkativeness, props.form.fav],
  ([propTalk, propFav]) => {
    ...
    if (
      typeof pTalk === "number" &&
      pTalk !== localOtherSettings.talkativeness
    ) {
      localOtherSettings.talkativeness = pTalk;
      changed = true;
    }
    if (typeof pFav === "boolean" && pFav !== localOtherSettings.fav) {
      localOtherSettings.fav = pFav;
      changed = true;
    }
    ...
    if (changed) {
      ...
      nextTick(() => {
        internalUpdateFlag_Other = false;
      });
    }
  },
  { immediate: true }
...
watch(
  ...
  (newVal) => {
    ...
    const pTalk =
      props.form.talkativeness === undefined ? 0.5 : props.form.talkativeness;
      ...
      emit("update:form", {
        ...
        data: {
          ...
          extensions: {
            ...
            talkativeness: String(newVal.talkativeness),
            fav: newVal.fav,
          },
        },
      });
...
<style scoped>
.other-settings-form-item {
  display: flex;
  flex-direction: column;
}
.other-settings-form-item :deep(.el-form-item__label) {
  text-align: left;
  line-height: 1.5;
  padding-bottom: 0.375rem;
  flex-shrink: 0;
}
.other-settings-form-item :deep(.el-form-item__content) {
...
.other-settings-slider {
  flex-grow: 1;
  min-width: 0;
}
.other-settings-slider :deep(.el-slider__runway) {
  flex: 1 1 auto;
  min-width: 40px;
  margin-right: 1rem;
}
.other-settings-slider :deep(.el-slider__input) {
  flex: 0 0 auto;
  width: 6.5rem;
}
.form-item-switch-control {
  margin-top: 0.25rem;
}
