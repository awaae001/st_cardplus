<template>
  <div class="content-panel-body space-y-4 md:space-y-5 h-full flex flex-col">
    ...
    <div class="flex-grow space-y-4 px-1">
      ...
      <el-form-item>
        <template #label><span class="form-label-adv">头像 (文件名或标识)</span></template>
        <el-input v-model="localForm.avatar" disabled placeholder="例如: char_avatar.png 或 none" />
      </el-form-item>
      <el-form-item>
        <template #label><span class="form-label-adv">第一条消息</span></template>
        <el-input type="textarea" v-model="localForm.first_mes" :autosize="{ minRows: 3, maxRows: 8 }" placeholder="请输入第一条消息" />
      </el-form-item>
    </div>
  </div>
</template>
...
import type { ElFormItem, ElInput } from 'element-plus';
...
interface Props { form: BasicInfoFormData; }

const props = defineProps<Props>();
const emit = defineEmits(['update:form']);

const localForm = reactive<Omit<BasicInfoFormData, 'data'>>({
  name: '', avatar: '', first_mes: ''
});
let internalUpdateFlag_BasicInfo = false;

watch(() => [props.form.name, props.form.avatar, props.form.first_mes], 
  ([newName, newAvatar, newFirstMes]) => {
    ...
    if (pName !== localForm.name) { localForm.name = pName; changed = true; }
    if (pAvatar !== localForm.avatar) { localForm.avatar = pAvatar; changed = true; }
    if (pFirstMes !== localForm.first_mes) { localForm.first_mes = pFirstMes; changed = true; }

    if (changed) {
      // console.log('BasicInfo: Props updated localForm', JSON.parse(JSON.stringify(localForm)));
      internalUpdateFlag_BasicInfo = true;
      nextTick(() => { internalUpdateFlag_BasicInfo = false; });
    }
  }, { immediate: true }
);
...
watch(localForm, (newVal) => {
  ...
  if (newVal.name !== (props.form.name || '') ||
