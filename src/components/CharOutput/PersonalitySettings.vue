  ...
  <div class="content-panel-body space-y-4 md:space-y-5 h-full flex flex-col">
    <div class="content-panel-header -mx-4 md:-mx-5 -mt-4 md:-mt-5 mb-5">
      <h3 class="content-panel-title flex items-center gap-2">
        <Icon icon="ph:smiley-sticker-duotone" class="text-xl text-accent-500 dark:text-accent-400"/>
        个性设定
      </h3>
    </div>
    <div class="flex-grow px-1 flex">
      <el-input type="textarea" v-model="localPersonality" :autosize="{ minRows: 5, maxRows: 12 }" placeholder="描述角色的性格特点、价值观、行为模式等" class="flex-grow"/>
    </div>
  </div>
</template>
...
interface PersonalitySettingsFormData { personality: string; data?: { personality?: string; [key: string]: any; }; }
interface Props { form: PersonalitySettingsFormData; }
...
watch(() => props.form.personality, (newVal) => {
  ...
  if (currentPropVal !== localPersonality.value) {
    ...
    nextTick(() => { internalUpdateFlag_Pers = false; });
  }
}, { immediate: true });
