<template>
  <div class="logic-block-panel">
    <div class="panel-header">
      <h3>逻辑块管理</h3>
      <el-button type="primary" :icon="Plus" @click="store.addLogicBlock" size="small">添加逻辑块</el-button>
    </div>

    <div v-if="store.logicBlocks.length === 0" class="empty-state">
      <el-empty description="暂无逻辑块" :image-size="60">
        <el-button type="primary" @click="store.addLogicBlock">创建第一个逻辑块</el-button>
      </el-empty>
    </div>
    ...
    <el-collapse v-model="activeBlockIds" class="logic-block-list">
      <el-collapse-item v-for="block in store.logicBlocks" :key="block.id" :name="block.id">
        ...
        <div class="block-actions">
            <el-button type="danger" :icon="Delete" @click.stop="removeBlock(block.id)" size="small">删除该逻辑块</el-button>
        </div>
      </el-collapse-item>
    </el-collapse>
  </div>
