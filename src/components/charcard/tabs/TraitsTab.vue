<template>
  <section class="form-section">
    ...
    <draggable
      :model-value="form.traits"
      @update:model-value="$emit('update:traits', $event)"
      handle=".drag-handle"
      item-key="index"
      animation="200"
      ghost-class="ghost"
    ...
    >
      <template #item="{ element: trait, index }">
        <el-card class="draggable-card">
          ...
          />
          <div
            v-for="(_, i) in trait.dialogueExamples"
            :key="i"
            class="cardInput"
          >
            <el-input
              v-model="trait.dialogueExamples[i]"
              type="textarea"
              :rows="2"
              :placeholder="`对话示例 ${Number(i) + 1}`"
            />
            <el-button
              @click="removeDialogueExample(index, Number(i))"
              size="small"
            >
              <Icon
          ...
          <el-button
            type="primary"
            @click="addDialogueExample(index)"
            size="small"
          ...
          <div
            v-for="(_, i) in trait.behaviorExamples"
            :key="i"
            class="cardInput"
          >
            <el-input
              v-model="trait.behaviorExamples[i]"
              type="textarea"
              :rows="2"
              :placeholder="`行为示例 ${Number(i) + 1}`"
            />
            <el-button
              @click="removeBehaviorExample(index, Number(i))"
              size="small"
            >
              <Icon
                icon="material-symbols:delete-outline"
          ...
          <el-button
            type="primary"
            @click="addBehaviorExample(index)"
            size="small"
            style="width: 100%; margin-top: 4px"
          >
            添加行为示例
  ...
  <section class="form-section">
    <div class="title-Btn-add form-section-title">
      ...
      <div style="display: flex; gap: 8px; margin-left: 16px">
        ...
        <el-button
          type="success"
          @click="$emit('exportRelationships')"
          title="导出人际关系"
        >
          <Icon
            icon="material-symbols:content-copy-outline"
    ...
    <draggable
      :model-value="form.relationships"
      @update:model-value="$emit('update:relationships', $event)"
      handle=".drag-handle"
      item-key="index"
      animation="200"
      ghost-class="ghost"
      ...
      <template #item="{ element: relationship, index }">
        <el-card class="draggable-card">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 1rem">
            ...
            <el-button
              type="primary"
              @click="relationship.dialogueExamples.push('')"
            >
              <Icon
                icon="material-symbols:desktop-landscape-add-outline"
                width="18"
