    ...
    >
      <template #item="{ element: skill }">
        <div :key="skill.id" class="skill-item-card-outer-wrapper">
          <div
            class="skill-item-card bg-white dark:bg-neutral-800 rounded-lg shadow-md dark:shadow-black/20 border border-neutral-200 dark:border-neutral-700 flex flex-col relative transition-all duration-150 ease-in-out hover:shadow-lg dark:hover:border-neutral-600 overflow-hidden"
          >
            <div
              class="skill-card-drag-handle bg-neutral-100 dark:bg-neutral-700/50 px-3 py-1.5 cursor-move flex items-center justify-between text-neutral-500 dark:text-neutral-400 border-b border-neutral-200 dark:border-neutral-700 text-xs"
            >
              <div
...
watch(
  ...
  (newSkillsFromProp) => {
    if (
      JSON.stringify(newSkillsFromProp) !== JSON.stringify(localSkills.value)
    ) {
      try {
...
watch(
  ...
  (newLocalState) => {
    if (
      JSON.stringify(newLocalState) !== JSON.stringify(props.form.skills || [])
    ) {
...
.skill-card-drag-handle
  :deep(.skill-name-input.el-textarea .el-textarea__inner) {
...
.skill-card-drag-handle
  :deep(.skill-name-input.el-textarea.is-focus .el-textarea__inner) {
...
.skill-item-card :deep(.el-textarea__inner) {
  font-size: 0.8125rem;
  line-height: 1.5;
  padding-top: 4px;
  padding-bottom: 4px;
