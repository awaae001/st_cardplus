  ...
  <div class="content-panel-body space-y-5">
    <div class="content-panel-header -mx-5 md:-mx-6 -mt-5 md:-mt-6 mb-6">
      ...
      <div class="flex gap-x-3 ml-auto">
        <button @click="props.addRelationship" class="btn-primary-adv text-sm !py-1.5 !px-3 whitespace-nowrap"
          title="添加新的人际关系">
          <Icon icon="material-symbols:add-circle-outline-rounded" width="18" height="18" class="mr-1.5 -ml-0.5" />
          添加关系
        </button>
        <button @click="props.exportRelationships" title="导出所有人际关系 (JSON)"
    ...
    <draggable v-if="localRelationships && localRelationships.length > 0" v-model="localRelationships" item-key="id"
      animation="200" ghost-class="relationship-ghost" chosen-class="relationship-chosen"
      handle=".relationship-card-drag-handle" class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 md:gap-5"
      ...
      <template #item="{ element: relationship, index: relationshipIndex }">
        <div :key="relationship.id" class="relationship-item-card-outer-wrapper">
          <div
            class="relationship-item-card bg-white dark:bg-neutral-800 rounded-lg shadow-md dark:shadow-black/20 border border-neutral-200 dark:border-neutral-700
            ...
            <div class="relationship-card-drag-handle bg-neutral-100 dark:bg-neutral-700/50 px-3 py-1.5 cursor-move
              ...
              <div class="flex items-center gap-1.5 flex-grow min-w-0" title="按住拖拽排序">
                ...
                <el-input v-model="relationship.name" type="textarea" :autosize="{ minRows: 1, maxRows: 2 }"
                  placeholder="角色名称" size="small" class="relationship-name-input flex-grow min-w-0"></el-input>
              </div>
              <button @click="() => props.removeRelationship(relationship.id)"
                ...
                title="删除此关系" :disabled="appSettings.safeModeLevel === 'forbidden'">
                <Icon icon="ph:x-bold" width="12" height="12" />
              </button>
            </div>

            <div class="p-3 flex flex-col gap-y-3 flex-grow">
              <el-input v-model="relationship.description" type="textarea" :autosize="{ minRows: 2, maxRows: 5 }"
                placeholder="关系描述" size="small"></el-input>
              <el-input v-model="relationship.features" type="textarea" :autosize="{ minRows: 1, maxRows: 4 }"
                placeholder="对方人物特征" size="small"></el-input>

              <div class="space-y-2">
                <label class="form-label-adv text-xs block mb-1">对话示例</label>
                <div v-for="(_dialogue, dialogueIndex) in relationship.dialogueExamples"
                  :key="`dialogue-${relationship.id}-${dialogueIndex}`" class="flex items-start gap-x-2">
                  <el-input v-model="relationship.dialogueExamples[dialogueIndex]" type="textarea"
                    :autosize="{ minRows: 1, maxRows: 4 }" :placeholder="`对话内容 ${dialogueIndex + 1}`" size="small"
                    class="flex-grow"></el-input>
                  <button @click="removeDialogueExample(relationshipIndex, dialogueIndex)"
                    class="btn-danger-adv !p-1.5 !aspect-square shrink-0 !text-xs mt-0.5 disabled:opacity-50 disabled:cursor-not-allowed"
                    title="移除此示例" :disabled="['forbidden', 'double'].includes(appSettings.safeModeLevel)">
                    <Icon icon="ph:trash-simple-duotone" width="14" height="14" />
                  </button>
                </div>
                <button @click="addDialogueExample(relationshipIndex)"
...
const handleDraggableChange = (event: { moved?: { oldIndex: number; newIndex: number } }) => {
  if (event.moved) {
    console.log(
      `关系 "${localRelationships.value[event.moved.newIndex].name}" 移动: ${event.moved.oldIndex} → ${event.moved.newIndex}`
    );
  }
};
</script>
...
.relationship-card-drag-handle :deep(.relationship-name-input.el-textarea .el-textarea__inner) {
...
:where(.dark, .dark *) .relationship-card-drag-handle :deep(.relationship-name-input.el-textarea .el-textarea__inner) {
  color: var(--color-neutral-300);
}

.relationship-card-drag-handle :deep(.relationship-name-input.el-textarea.is-focus .el-textarea__inner) {
  background-color: var(--color-white);
}

:where(.dark, .dark *) .relationship-card-drag-handle :deep(.relationship-name-input.el-textarea.is-focus .el-textarea__inner) {
