...
// --- Script部分与你上一版本AttireSettings.vue中的保持一致 (包含所有接口、props、emits、refs、watchers、methods) ---
// --- 确保 removeAttire prop 的类型是 (attireId: string) => void ---
...
interface FieldHeights {
  name?: number; description?: number; tops?: number; bottoms?: number; shoes?: number; socks?: number; underwears?: number; accessories?: number;
}
interface AttireInternalLayoutState { fieldGroupMode: 'grid' | 'stacked'; fieldHeights: FieldHeights; }

interface AttireItem {
  id: string; name: string; description: string; tops: string; bottoms: string; shoes: string; socks: string; underwears: string; accessories: string;
...
interface Props {
  ...
  addAttire: () => void;
  removeAttire: (attireId: string) => void; // MODIFIED: parameter type
...
const createDefaultLayoutState = (): AttireInternalLayoutState => ({
  fieldGroupMode: 'grid',
  fieldHeights: reactive({ name: 0, description: 0, tops: 0, bottoms: 0, shoes: 0, socks: 0, underwears: 0, accessories: 0 }),
});

watch(() => props.form.attires, (newAttiresFromProp) => {
  const newAttiresJson = JSON.stringify(newAttiresFromProp.map(a => ({...a, id: a.id || ''})));
  const currentAttiresJson = JSON.stringify(localAttiresReactive.value.map(a => { const {_internalLayoutState, ...rest} = a; return {...rest, id: rest.id || ''}; }));
  if (newAttiresJson !== currentAttiresJson) {
    try {
      const processedAttires = (newAttiresFromProp && Array.isArray(newAttiresFromProp) ? JSON.parse(JSON.stringify(newAttiresFromProp)) : [] )
      .map((attireData: Omit<AttireItem, '_internalLayoutState'>) => {
        const existingItem = localAttiresReactive.value.find(item => item.id === attireData.id);
        return { ...attireData, _internalLayoutState: existingItem ? existingItem._internalLayoutState : reactive(createDefaultLayoutState()) };
      });
      localAttiresReactive.value = processedAttires;
      nextTick(async () => { await initializeAllTextareaLayouts(); });
    } catch (e) { console.error('[AttireSettings] Error processing attires from props:', e); localAttiresReactive.value = []; }
  }
}, { deep: true, immediate: true });

watch(localAttiresReactive, (newLocalAttires) => {
  const attiresToEmit = newLocalAttires.map(attire => { const { _internalLayoutState, ...restOfAttire } = attire; return restOfAttire; });
  const currentPropAttiresForEmit = props.form.attires || [];
  if (JSON.stringify(attiresToEmit) !== JSON.stringify(currentPropAttiresForEmit)) {
...
const initializeAllTextareaLayouts = async () => {
  for (const attire of localAttiresReactive.value) {
    await nextTick(); 
    const attireCardElement = document.querySelector(`.attire-item-card-outer-wrapper div[key="${attire.id}"] .attire-item-card`);
    if (attireCardElement) {
      const textareas = attireCardElement.querySelectorAll('.expanding-textarea textarea, .attire-name-input-in-handle textarea') as NodeListOf<HTMLTextAreaElement>; // Include name input
      textareas.forEach(textarea => {
        if (textarea.value) { 
            const placeholder = textarea.placeholder.toLowerCase();
            const isNameInput = textarea.classList.contains('el-textarea__inner') && textarea.parentElement?.parentElement?.classList.contains('attire-name-input-in-handle');
            ...
            const isDynamicGroupMember = fieldKey && ['tops', 'bottoms', 'shoes', 'socks'].includes(fieldKey);
...
const updateFieldHeight = ( attire: AttireItem, fieldKey: LayoutFieldKey | OtherFieldKey, textareaElement: HTMLTextAreaElement | null, partOfDynamicGroup: boolean = false ) => {
  ...
  if (partOfDynamicGroup) {
    ...
    for (const key of groupFields) {
      if ((attire._internalLayoutState.fieldHeights[key] || 0) > HEIGHT_THRESHOLD_FOR_STACKING) {
        shouldStack = true; break;
      }
    }
    if (shouldStack) {
      if (attire._internalLayoutState.fieldGroupMode !== 'stacked') attire._internalLayoutState.fieldGroupMode = 'stacked';
    } else {
      if (attire._internalLayoutState.fieldGroupMode !== 'grid') attire._internalLayoutState.fieldGroupMode = 'grid';
    }
  }
};

const getFieldGroupLayoutClass = (attire: AttireItem): string => {
  if (attire._internalLayoutState?.fieldGroupMode === 'stacked') return "grid grid-cols-1 gap-y-2.5";
  return "grid grid-cols-1 sm:grid-cols-2 gap-x-3 gap-y-2.5"; 
};

const handleDraggableChange = (event: any) => { /* Watcher handles emit */ };
</script>

<style scoped>
/* Ghost and Chosen styles (与上一版 AttireSettings.vue 相同) */
.attire-ghost { opacity: 0.6; border-radius: var(--radius-lg); background-color: var(--color-sky-100); outline: 2px dashed var(--color-sky-400); box-shadow: none; }
:where(.dark, .dark *) .attire-ghost { background-color: oklch(from var(--color-sky-800) l c h / 0.4); }
.attire-chosen .attire-item-card { --chosen-ring-width: 2px; --chosen-ring-offset-width: 2px; --chosen-ring-color-light: var(--color-accent-500); --chosen-ring-offset-color-light: var(--color-white); --chosen-ring-color-dark: var(--color-accent-400); --chosen-ring-offset-color-dark: var(--color-neutral-850); box-shadow: 0 0 0 var(--chosen-ring-offset-width) var(--chosen-ring-offset-color-light), 0 0 0 calc(var(--chosen-ring-offset-width) + var(--chosen-ring-width)) var(--chosen-ring-color-light), var(--shadow-xl); }
:where(.dark, .dark *) .attire-chosen .attire-item-card { box-shadow: 0 0 0 var(--chosen-ring-offset-width) var(--chosen-ring-offset-color-dark), 0 0 0 calc(var(--chosen-ring-offset-width) + var(--chosen-ring-width)) var(--chosen-ring-color-dark), var(--shadow-xl); }
.attire-chosen { transform: scale(1.02); z-index: 10; }

.attire-card-drag-handle:hover { background-color: var(--color-neutral-200); }
:where(.dark, .dark *) .attire-card-drag-handle:hover { background-color: var(--color-neutral-600); }

/* MODIFIED: Styles for name input in handle */
.attire-card-drag-handle :deep(.attire-name-input-in-handle.el-textarea .el-textarea__inner) {
  ...
  resize: none;
  text-align: center; /* 让名称在手柄中居中 */
}
:where(.dark, .dark *) .attire-card-drag-handle :deep(.attire-name-input-in-handle.el-textarea .el-textarea__inner) {
  color: var(--color-neutral-300);
}
.attire-card-drag-handle :deep(.attire-name-input-in-handle.el-textarea.is-focus .el-textarea__inner) {
  background-color: var(--color-white); 
}
:where(.dark, .dark *) .attire-card-drag-handle :deep(.attire-name-input-in-handle.el-textarea.is-focus .el-textarea__inner) {
  background-color: var(--color-neutral-700); 
}

/* Styles for all other expanding textareas in the card body */
.attire-item-card .p-3 :deep(.expanding-textarea.el-textarea .el-textarea__inner) { /* Target only textareas in p-3 */
  font-size: 0.8125rem; 
  line-height: 1.5; 
  padding-top: 4px; 
  padding-bottom: 4px;
...
.attire-item-card :deep(.grid .expanding-textarea.el-textarea) { width: 100%; }
</style>
