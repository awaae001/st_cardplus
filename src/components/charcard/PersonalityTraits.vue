    ...
    >
      <template #item="{ element: trait, index: traitIndex }">
        <div :key="trait.id" class="trait-item-card-outer-wrapper">
          <div
            class="trait-item-card bg-white dark:bg-neutral-800 rounded-lg shadow-md dark:shadow-black/20 border border-neutral-200 dark:border-neutral-700 flex flex-col relative transition-all duration-150 ease-in-out hover:shadow-lg dark:hover:border-neutral-600 overflow-hidden"
          >
            <div
              class="trait-card-drag-handle bg-neutral-100 dark:bg-neutral-700/50 px-3 py-1.5 cursor-move flex items-center justify-between text-neutral-500 dark:text-neutral-400 border-b border-neutral-200 dark:border-neutral-700 text-xs"
            >
              <div
              ...
              >
                ...
                <el-input
                  ...
                  placeholder="特质名称"
                  size="small"
                  class="trait-name-input flex-grow min-w-0"
                ></el-input>
              </div>
              <button
            ...
            <div class="p-3 flex flex-col gap-y-3 flex-grow">
              <el-input
                ...
                placeholder="特质描述 (详细说明该特质的表现和影响)"
                size="small"
              ></el-input>
              <div class="space-y-2">
                ...
                >
                <div
                  v-for="(_dialogue, dialogueIndex) in trait.dialogueExamples"
                  :key="`dialogue-${trait.id}-${dialogueIndex}`"
                  class="flex items-start gap-x-2"
                >
                  <el-input
                    v-model="trait.dialogueExamples[dialogueIndex]"
                    type="textarea"
                    :autosize="{ minRows: 1, maxRows: 4 }"
                    :placeholder="`示例 ${dialogueIndex + 1}`"
                    size="small"
                    class="flex-grow"
                  ></el-input>
                  <button
                ...
                <button
                  @click="addDialogueExample(traitIndex)"
              ...
              <div class="space-y-2">
                ...
                >
                <div
                  v-for="(_behavior, behaviorIndex) in trait.behaviorExamples"
                  :key="`behavior-${trait.id}-${behaviorIndex}`"
                  class="flex items-start gap-x-2"
                >
                  <el-input
                    v-model="trait.behaviorExamples[behaviorIndex]"
                    type="textarea"
                    :autosize="{ minRows: 1, maxRows: 4 }"
                    :placeholder="`示例 ${behaviorIndex + 1}`"
                    size="small"
                    class="flex-grow"
                  ></el-input>
                  <button
                ...
                <button
                  @click="addBehaviorExample(traitIndex)"
                  class="btn-secondary-adv w-full !py-1 !px-2 text-xs"
                >
                  <Icon icon="ph:plus-circle-duotone" class="mr-1 text-sm" />
                  添加行为示例
...
watch(
  ...
  (newTraitsFromProp) => {
    if (
      JSON.stringify(newTraitsFromProp) !== JSON.stringify(localTraits.value)
    ) {
      try {
        localTraits.value = JSON.parse(JSON.stringify(newTraitsFromProp || []));
      } catch (e) {
        console.error(
          "[PersonalityTraits] Cloning traits from props failed:",
          e
        );
        localTraits.value = [];
      }
...
watch(
  ...
  (newLocalTraitsState) => {
    if (
      JSON.stringify(newLocalTraitsState) !==
      JSON.stringify(props.form.traits || [])
    ) {
...
const removeDialogueExample = async (
  ...
  if (
    !trait ||
    !trait.dialogueExamples ||
    exampleIndex < 0 ||
    exampleIndex >= trait.dialogueExamples.length
  )
    return;
  const exampleText =
    trait.dialogueExamples[exampleIndex].substring(0, 20) ||
    `示例 ${exampleIndex + 1}`;
...
const removeBehaviorExample = async (
  traitIndex: number,
  exampleIndex: number
) => {
  const trait = localTraits.value[traitIndex];
  if (
    !trait ||
    !trait.behaviorExamples ||
    exampleIndex < 0 ||
    exampleIndex >= trait.behaviorExamples.length
  )
    return;
  const exampleText =
    trait.behaviorExamples[exampleIndex].substring(0, 20) ||
    `示例 ${exampleIndex + 1}`;
...
const handleDraggableChange = (event: {
  ...
  if (event.moved) {
    console.log(
      `关系 "${localTraits.value[event.moved.newIndex].name}" 移动: ${
...
.trait-card-drag-handle
  :deep(.trait-name-input.el-textarea .el-textarea__inner) {
...
.trait-card-drag-handle
  :deep(.trait-name-input.el-textarea.is-focus .el-textarea__inner) {
...
.trait-item-card :deep(.el-textarea__inner) {
  font-size: 0.8125rem;
  line-height: 1.5;
  padding-top: 4px;
  padding-bottom: 4px;
