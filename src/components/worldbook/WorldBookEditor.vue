<template>
  <el-scrollbar class="worldbook-editor-scrollbar">
    <div class="content-panel-body">
      ...
      <el-form v-if="entry && modelValue" :model="localModel" label-position="top" ref="entryFormRef" class="worldbook-editor-form">
        <section class="form-section">
          ...
          <div class="form-section-content">
            <div>
              <label class="form-label">标题/备注 (Comment)</label><el-input v-model="localModel.comment" placeholder="给条目起个易于识别的名字" />
            </div>
            <div>
              <label class="form-label">主要关键词 (Key) -
                <span class="form-label-subtext">逗号分隔, 支持 /regex/i</span></label><el-input v-model="keysInput" type="textarea" :autosize="{ minRows: 2, maxRows: 4 }" placeholder="例如: 角色名, /他说了什么/i, 地点A" />
              <p class="form-help-text">
                提示: 普通文本关键词不要包含逗号，逗号被视作分隔符。
              </p>
            </div>
            <div>
              <label class="form-label">条目内容 (Content)</label><el-input v-model="localModel.content" type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" placeholder="当条目激活时，这段文本会被插入到AI的提示中..." />
            </div>
          ...
          <div class="form-grid-2-col">
            <div>
              <label class="form-label">次要关键词 (Optional Filter) -
                <span class="form-label-subtext">逗号分隔</span></label><el-input v-model="secondaryKeysInput" type="textarea" :autosize="{ minRows: 2, maxRows: 3 }" placeholder="可选的过滤关键词" />
            </div>
            <div>
              <label class="form-label">次要关键词逻辑 (Logic)</label><el-select v-model="localModel.selectiveLogic" placeholder="选择逻辑" class="form-full-width" :disabled="!localModel.selective"><el-option label="与任意" :value="0" /><el-option label="非所有" :value="1" /><el-option label="非任何" :value="2" /><el-option label="与所有" :value="3" /></el-select><el-checkbox v-model="localModel.selective" label="启用次要逻辑" class="form-checkbox-margin-top" />
            </div>
            ...
            <div class="form-grid-span-2">
              ...
              <div class="form-flex-row">
                <el-slider v-model="localModel.probability" :min="0" :max="100" show-input class="form-slider" :disabled="!localModel.useProbability" /><el-checkbox v-model="localModel.useProbability" label="启用概率" class="form-checkbox-nowrap" />
              </div>
            </div>
          </div>
        </section>
        <section class="form-section">
          ...
          <div class="form-grid-3-col">
            <div>
              <label class="form-label">插入顺序 (Order)</label><el-input-number v-model="localModel.order" :min="0" controls-position="right" class="form-full-width" />
            </div>
            <div>
              <label class="form-label">插入位置 (Position)</label><el-select v-model="localModel.position" placeholder="选择插入位置" class="form-full-width"><el-option label="角色定义之前" :value="0" />
            ...
            <div v-if="localModel.position === 4">
              <label class="form-label">深度角色 (Role for In-chat)</label><el-select v-model="localModel.role" placeholder="选择角色" class="form-full-width"><el-option label="系统" :value="0" />
        ...
        <section class="form-section">
          ...
          <div class="form-grid-3-col-top-align">
            <div>
              <label class="form-label">扫描深度 (Scan Depth)</label><el-input-number v-model="localModel.depth" :min="0" :max="999" controls-position="right" class="form-full-width" />
        ...
        <section class="form-section">
          ...
          <div class="form-grid-3-col-top-align">
            ...
            <div>
              <label class="form-label">所属收录组 (Group)</label><el-input v-model="localModel.group" placeholder="组名, 多个用逗号分隔" />
            </div>
            <div>
              <label class="form-label">组内优先级/权重</label><el-input-number v-model="localModel.groupPriority" controls-position="right" class="form-full-width" />
            </div>
            <div class="form-flex-col-start">
        ...
        <section class="form-section">
          ...
          <div class="form-grid-3-col-end-align">
            <div>
              <label class="form-label">粘滞回合数 (Sticky)</label><el-input-number v-model="localModel.sticky" :min="0" controls-position="right" class="form-full-width" />
              <p class="form-help-text">0表示不粘滞</p>
            </div>
            <div>
              <label class="form-label">冷却回合数 (Cooldown)</label><el-input-number v-model="localModel.cooldown" :min="0" controls-position="right" class="form-full-width" />
              <p class="form-help-text">0表示无冷却</p>
            </div>
            <div>
              <label class="form-label">延迟激活 (Delay)</label><el-input-number v-model="localModel.delay" :min="0" controls-position="right" class="form-full-width" />
        ...
        <section class="form-section">
          ...
          <div>
            <label class="form-label">自动化ID (Automation ID)</label><el-input v-model="localModel.automationId" placeholder="用于Quick Replies扩展" />
          </div>
        </section>
      </el-form>
    </div>
