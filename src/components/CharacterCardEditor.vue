...
const removeTraitById = async (traitId: string): Promise<void> => {
    ...
    await performSafeAction(appSettings.safeModeLevel, '移除特质', traitName, () => {
        const currentIndex = form.value.traits.findIndex(t => t.id === traitId);
        if (currentIndex !== -1) {
             form.value.traits.splice(currentIndex, 1);
        }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('移除特质操作未成功完成:', err);});
};
...
const removeSkillById = async (skillId: string): Promise<void> => {
    ...
    await performSafeAction(appSettings.safeModeLevel, '移除技能', skillName, () => {
        const currentIndex = form.value.skills.findIndex(s => s.id === skillId);
         if (currentIndex !== -1) {
            form.value.skills.splice(currentIndex, 1);
         }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('移除技能操作未成功完成:', err);});
};
...
const removeRelationshipById = async (relationshipId: string): Promise<void> => {
    ensureArrayExists('relationships');
    const index = form.value.relationships.findIndex(r => r.id === relationshipId);
    if (index === -1) return;
    const relationshipName = form.value.relationships[index].name || '该关系';
    await performSafeAction(appSettings.safeModeLevel, '移除关系', relationshipName, () => {
        const currentIndex = form.value.relationships.findIndex(r => r.id === relationshipId);
         if (currentIndex !== -1) {
             form.value.relationships.splice(currentIndex, 1);
         }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('移除关系操作未成功完成:', err);});
};
...
const removeAttireById = async (attireId: string): Promise<void> => {
    ...
    await performSafeAction(appSettings.safeModeLevel, '移除套装', attireName, () => {
        const currentIndex = form.value.attires.findIndex(a => a.id === attireId);
        if (currentIndex !== -1) {
            form.value.attires.splice(currentIndex, 1);
        }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('移除套装操作未成功完成:', err);});
};

const handleRemoveCustomField = async (fieldLabel: string) => {
    await performSafeAction(appSettings.safeModeLevel, '移除自定义外观字段', fieldLabel, () => {
      if (form.value.appearance && Object.prototype.hasOwnProperty.call(form.value.appearance, fieldLabel)) {
        delete form.value.appearance[fieldLabel];
      } else {
         console.warn(`Attempted to remove non-existent custom field via safe action: ${fieldLabel}`);
         throw new Error('字段未找到。');
      }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('移除自定义字段操作未成功完成:', err);});
};
...
const stripIdsFromArray = <T extends { id: string }>(arr: T[]): Omit<T, 'id'>[] => {
...
const getDataForExport = (): any => {
  const dataToSerialize = {
    ...
    attires: form.value.attires.map(attire => ({ ...attire, accessories: processTextToArray(attire.accessories) }))
  };
...
const saveCharacterCard = async (): Promise<void> => {
    try {
        ...
        const characterNameForFile = (form.value.chineseName || form.value.name || '角色卡').replace(/[<>:"/\\|?*]+/g, '_');
...
const loadCharacterCard = async (): Promise<void> => {
    await performSafeAction(appSettings.safeModeLevel, '加载角色卡', '此操作将覆盖当前内容', async () => {
      ...
      await new Promise<void>((resolve, reject) => {
          input.onchange = async (event) => {
              ...
              try {
                  ...
                  if (!parsedData || (typeof parsedData !== 'object')) { throw new Error('无效的角色卡文件格式。'); }
   ...
   }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('加载角色卡操作未成功完成:', err);});
};
...
const copyToClipboard = async (): Promise<void> => {
    try {
        ...
        ElMessage.success({ message: '角色卡数据已复制到剪贴板 (列表项无ID)！', duration: 2000 });
    } catch (error) {
...
const importFromClipboard = async (data: string): Promise<void> => {
     if(!data.trim()) { ElMessage.warning("剪贴板数据为空。"); return; }
     await performSafeAction(appSettings.safeModeLevel, '从剪贴板导入', '此操作将覆盖当前内容', () => {
        try {
            const parsedData = JSON.parse(data);
            if (!parsedData || (typeof parsedData !== 'object')) { throw new Error('剪贴板数据不是有效的角色卡JSON格式。'); }
            form.value = processLoadedData(parsedData);
            saveToLocalStorage(form.value);
        } catch (error) {
             throw new Error(`导入处理失败：${error instanceof Error ? error.message : '未知错误'}`);
        }
    }).catch(err => { if(err !== 'cancel' && err !== 'forbidden') console.warn('从剪贴板导入操作未成功完成:', err);});
};


const exportAttires = async (): Promise<void> => { ensureArrayExists('attires'); if (!form.value.attires || form.value.attires.length === 0) { ElMessage.warning('没有服装数据可导出。'); return; } const attireDataToExport = form.value.attires.map(a => { const {id, ...rest} = a; return {...rest, accessories: processTextToArray(a.accessories)}; }); try { await navigator.clipboard.writeText(JSON.stringify(attireDataToExport, null, 2)); ElMessage.success('服装数据已复制到剪贴板 (无ID)！'); } catch (err) { ElMessage.error('复制服装数据失败。'); }};
const exportSkills = async (): Promise<void> => { ensureArrayExists('skills'); if (!form.value.skills || form.value.skills.length === 0) { ElMessage.warning('没有技能数据可导出。'); return; } const skillsToExport = stripIdsFromArray(form.value.skills); try { await navigator.clipboard.writeText(JSON.stringify(skillsToExport, null, 2)); ElMessage.success('技能数据已复制到剪贴板 (无ID)！'); } catch (err) { ElMessage.error('复制技能数据失败。'); }};
const exportTraits = async (): Promise<void> => { ensureArrayExists('traits'); if (!form.value.traits || form.value.traits.length === 0) { ElMessage.warning('没有特质数据可导出。'); return; } const traitsToExport = stripIdsFromArray(form.value.traits); try { await navigator.clipboard.writeText(JSON.stringify(traitsToExport, null, 2)); ElMessage.success('性格特质数据已复制到剪贴板 (无ID)！'); } catch (err) { ElMessage.error('复制特质数据失败。'); }};
const exportRelationships = async (): Promise<void> => { ensureArrayExists('relationships'); if (!form.value.relationships || form.value.relationships.length === 0) { ElMessage.warning('没有人际关系数据可导出。'); return; } const relationshipsToExport = stripIdsFromArray(form.value.relationships); try { await navigator.clipboard.writeText(JSON.stringify(relationshipsToExport, null, 2)); ElMessage.success('人际关系数据已复制到剪贴板 (无ID)！'); } catch (err) { ElMessage.error('复制关系数据失败。'); }};
