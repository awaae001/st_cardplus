<template>
  <div
    class="character-output-editor p-3 md:p-5 h-full flex flex-col print:p-0 print:bg-white print:text-black"
  >
    <header
...
const handleFormUpdate = (updatedPart: Partial<CharacterOutputForm>) => {
  ...
  for (const key in updatedPart) {
    if (Object.prototype.hasOwnProperty.call(updatedPart, key)) {
      if (
        key === "data" &&
        typeof updatedPart.data === "object" &&
        updatedPart.data !== null
      ) {
        newFormState.data = { ...newFormState.data, ...updatedPart.data };
        if (
          typeof updatedPart.data.extensions === "object" &&
          updatedPart.data.extensions !== null
        ) {
...
const processLoadedData = (parsedData: any): CharacterOutputForm => {
  ...
  if (parsedData.talkativeness !== undefined) {
    ...
    processed.talkativeness =
      Number(parsedData.data.extensions.talkativeness) || 0.5;
    processed.data.extensions.talkativeness = String(processed.talkativeness);
  } else {
    processed.talkativeness = defaultForm.talkativeness;
    processed.data.extensions.talkativeness =
      defaultForm.data.extensions.talkativeness;
  }
  ...
  fieldsToEnsureArray.forEach((field) => {
    if (field === "tags") {
      processed.tags = Array.isArray(parsedData.tags)
        ? parsedData.tags
        : Array.isArray(parsedData.data?.tags)
        ? parsedData.data.tags
        : [];
      processed.data.tags = processed.tags;
    } else if (Object.prototype.hasOwnProperty.call(processed.data, field)) {
      (processed.data as any)[field] = Array.isArray(parsedData.data?.[field])
...
const saveData = () => {
  const dataToSave: CharacterOutputForm = JSON.parse(
  ...
  const filename = `character_export_${(dataToSave.name || "untitled").replace(
    /[<>:"/\\|?*]+/g,
    "_"
  )}_${Date.now()}.json`;
          ...
          reader.onload = (ev) => {
            try {
              if (!ev.target?.result || typeof ev.target.result !== "string")
                throw new Error("无法读取或文件内容无效。");
              if (!form.value.data) form.value.data = createDefaultForm().data;
              if (!form.value.data.extensions)
                form.value.data.extensions =
                  createDefaultForm().data.extensions;
              if (!form.value.data.extensions.depth_prompt)
                form.value.data.extensions.depth_prompt =
                  createDefaultForm().data.extensions.depth_prompt;
        ...
        input.onchange = () => {
          ...
          reader.onload = (ev) => {
            try {
              ...
              if (form.value.data)
                form.value.data.description = ev.target.result;
              saveToLS(form.value, LOCAL_STORAGE_KEY);
              resolve();
            } catch (error) {
