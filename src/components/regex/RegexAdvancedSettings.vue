<template>
  ...
  <el-row :gutter="20">
    <el-col :span="12">
      <el-form-item label="应用位置 (placement)">
        ...
        >
          <el-checkbox
            v-for="(value, key) in REGEX_PLACEMENT"
            :key="value"
            :label="value"
          >
            {{ key }}
    ...
    <el-col :span="12">
      <el-form-item label="替换模式 (substituteRegex)">
        <el-select
          :model-value="modelValue.substituteRegex"
          @update:model-value="update('substituteRegex', $event)"
        >
          <el-option
            v-for="(value, key) in SUBSTITUTE_FIND_REGEX"
            :key="value"
            :label="key"
            :value="value"
          ></el-option>
    ...
    <el-col :span="8">
      <el-form-item label="最小深度 (minDepth)">
        <el-input-number
          ...
          @update:model-value="update('minDepth', $event)"
          :controls="false"
          placeholder="无限制"
          style="width: 100%"
        />
    ...
    <el-col :span="8">
      <el-form-item label="最大深度 (maxDepth)">
        <el-input-number
          ...
          @update:model-value="update('maxDepth', $event)"
          :controls="false"
          placeholder="无限制"
          style="width: 100%"
        />
  ...
  <el-form-item label="选项">
    ...
    ></el-checkbox>
    <el-checkbox
      :model-value="modelValue.markdownOnly"
      @update:model-value="update('markdownOnly', $event)"
      label="仅Markdown (markdownOnly)"
    ></el-checkbox>
    <el-checkbox
      :model-value="modelValue.promptOnly"
      @update:model-value="update('promptOnly', $event)"
      label="仅Prompt (promptOnly)"
    ></el-checkbox>
    <el-checkbox
      :model-value="modelValue.runOnEdit"
      @update:model-value="update('runOnEdit', $event)"
      label="编辑时运行 (runOnEdit)"
    ></el-checkbox>
  </el-form-item>
</template>
