...
export function useSmartRegexGenerator(originalText: MaybeRef<string>) {
    ...
    try {
      ...
      sortedSelections.forEach((selection) => {
        ...
        if (
          ...
          selection.type === 'end' ||
          selection.startIndex < actualStartIndex ||
          selection.endIndex > actualEndIndex
        ) {
          return;
        }
  ...
  const addSelection = (selection: TextSelection) => {
    ...
    // 检查是否有重叠
    const hasOverlap = selections.value.some(
      (sel) =>
  ...
  const validateSelections = computed(() => {
    ...
    if (hasStart || hasEnd) {
      ...
      const invalidSelections = selections.value.filter(
        (sel) => sel.type !== 'start' && sel.type !== 'end' && (sel.startIndex < startIndex || sel.endIndex > endIndex)
      );
